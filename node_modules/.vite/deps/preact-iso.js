...
function handleNav(state, action) {
  ...
  if (action && action.type === "click") {
    ...
    if (!link || link.origin != location.origin || /^#/.test(href) || !/^(_?self)?$/i.test(link.target) || !isInScope(href) || link.download) {
      return state;
    }
    push = true;
...
function LocationProvider(props) {
  ...
  return k(LocationProvider.ctx.Provider, { value }, props.children);
}
...
function Router(props) {
  ...
  F(props.children).some((vnode) => {
    ...
    if (matches) return pathRoute = W(vnode, matchProps);
    if (vnode.props.default) defaultRoute = W(vnode, matchProps);
  });
  ...
  const routeChanged = T(() => {
    ...
    k(RouteContext.Provider, { value: matchProps }, incoming);
  ...
  return routeChanged ? [k(RenderRef, { r: cur }), k(RenderRef, { r: prev })] : k(RenderRef, { r: cur });
}
var MODE_HYDRATE = 1 << 5;
var MODE_SUSPENDED = 1 << 7;
var RenderRef = ({ r }) => r.current;
Router.Provider = LocationProvider;
LocationProvider.ctx = X(
...
var Route = (props) => k(props.component, props);
...
function prerender(vnode, options) {
  return import("./prerender-KMGQ75NZ.js").then((m) => m.default(vnode, options));
}
export {
  ErrorBoundary,
  LocationProvider,
