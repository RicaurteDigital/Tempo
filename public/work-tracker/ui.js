...
const WorkTracker = (() => {
  ...
  function _showAddShift(dateStr) {
    ...
    ov.querySelector('#wt-clockin-now').onclick = () => {
      ...
      WTDb.saveShift({ id: generateId(), date: dateStr, locationId: locId, locationName: loc.name, hourlyRate: rate, shiftType, entries: [{ id: generateId(), clockIn: new Date().toISOString(), clockOut: null, breakMinutes: 0 }] });
      ov.remove();
      _go('home');
    };
  }
